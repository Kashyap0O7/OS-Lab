...
long long N_PRIME = 1000000000000;
int SQRT_PRIME = 1000000;
...
procinit(void)
{
  ...
  /**********************  Custom code begins  *********************/
  for(int i = 0; i < SQRT_PRIME; i++) is_prime[i] = 1;
  
  is_prime[0] = 0;
  is_prime[1] = 0;
  for(long long i = 2; i < SQRT_PRIME; i++)
  {
    if(is_prime[i] == 0) continue;
    for(long long j = i*i; j < SQRT_PRIME; j += i) is_prime[j] = 0;
  }
...
{
  if(x < SQRT_PRIME) return is_prime[x];
...
{
  for(int i = x + 1; ; i++) if(check_prime(i)) return i;
  return -1;
}
